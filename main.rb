...
# A new game!
new_game = { name: "New Exciting Game",
...
puts append_to_string("The game #{games_array.first[:name]} is ranked #{games_array.first[:overall_rank]}")
...
if games_array.any? { |game| game[:year] == 2024 }
  puts 'There are games from 2024 in the list.'
elsif games_array.all? { |game| game[:year] < 2024 }
...
def list_designers(&block)
  block.call(games_array.map { |game| game[:designer] }.uniq.compact.join(', '))
end

list_designers { |designers| puts "Game Designers: #{designers}" }
